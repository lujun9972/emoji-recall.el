...
3. 接下来再定义个函数画出第N轮的问题
   ...
   #+END_SRC
** option-buffer
在opton-buffer中需要列出所有可能的emoji供玩家选择,玩家点击一个emoji则在 =answer-buffer= 中存放答案的区域添加一个emoji,当然为了放置玩家输错,需要允许玩家点击答案区域的emoji撤回该emoji.
...
1. 然后定义option区域中emoji button的类型(暂时命名为option-button吧),该类型的button要在点击的时候,在答案区域插入一个answer-button.
   #+BEGIN_SRC emacs-lisp
...
#+BEGIN_SRC emacs-lisp
  ...
  (defun emoji-recall-game-over ()
    ...
    (erase-buffer)
    (insert "Congratulations! You Got Round %d" emoji-recall-round)
    (newline)
    (insert "Press any key to quit !")
    (read-char)
